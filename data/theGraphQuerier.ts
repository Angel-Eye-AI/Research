...
const tryFetchPairAddress = async (token0: string, token1: string): Promise<string | null> => {
  const query = gql`
  ...
  try {
    const data = await request<PairData>(UNISWAP_V2_SUBGRAPH_URL, query, variables);
...
const isV2Pair = async (address: string): Promise<boolean> => {
...
async function getPairAddress(token0: string, token1: string): Promise<string | null> {
...
const getNonWETHTokenAddress = async (pairAddress: string): Promise<string | null> => {
    // ABI definition of the UniswapV2Pair contract
