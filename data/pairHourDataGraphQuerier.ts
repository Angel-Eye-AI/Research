...
async function writeCSVLine(
  ...
  fs.appendFileSync(OUTPUT_CSV, 
    `${hourStartUnix},${pairAddress},${reserve0},${reserve1},${reserveUSD},${hourlyVolumeToken0},${hourlyVolumeToken1},${hourlyVolumeUSD},${hourlyTxns},${price}\n`
  );
}
...
async function fetchData(pairAddress: string) {
  try {
    const data: any = await request(UNISWAP_V2_SUBGRAPH_URL, GET_PAIR_DATA, { pairAddress });

    for (const pairHourData of data.pairHourDatas) {
        const { hourStartUnix, pair, reserve0, reserve1, token1, token0, 
            reserveUSD, hourlyVolumeToken0, hourlyVolumeToken1, hourlyVolumeUSD, hourlyTxns } = pairHourData;
        ...
        if (wethIsToken0) {
            ...
            await writeCSVLine(
                pairAddress,
                hourStartUnix,
                reserve0, //always put weth token data in token0 spor
                reserve1, //reversed below
                reserveUSD,
                hourlyVolumeToken0, //reversed below
                hourlyVolumeToken1, //reversed below
                hourlyVolumeUSD,
                hourlyTxns,
                nonWethTokenPriceInWeth
            );
        } else {
            //await writeData(
            await writeCSVLine(
                pairAddress,
                hourStartUnix,
                reserve1, //always put weth token data in token0 spor
                reserve0, 
                reserveUSD,
                hourlyVolumeToken1,
                hourlyVolumeToken0,
